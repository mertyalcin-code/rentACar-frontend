...
export class PaymentService {
  ...
  findAll(): Observable<ListResponseModel<PaymentListModel>> {
    return this.httpClient.get<ListResponseModel<PaymentListModel>>(
      this.apiUrl + 'find-all'
    );
  }
  findAllByRentalId(
    rentalId: number
  ): Observable<ListResponseModel<PaymentListModel>> {
    return this.httpClient.get<ListResponseModel<PaymentListModel>>(
      this.apiUrl + 'find-all-by-rental-id/' + rentalId
    );
  }
  findById(id: number): Observable<SingleResponseModel<PaymentListModel>> {
    return this.httpClient.get<SingleResponseModel<PaymentListModel>>(
  ...
  calculateTotalPrice(
    totalPriceRequestModel: TotalPriceRequestModel
  ): Observable<SingleResponseModel<number>> {
    return this.httpClient.post<SingleResponseModel<number>>(
      this.apiUrl + 'find-total-price/',
      totalPriceRequestModel
    );
  }
}
