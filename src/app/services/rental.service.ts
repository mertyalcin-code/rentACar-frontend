...
export class RentalService {
  ...
  findAll(): Observable<ListResponseModel<RentalListModel>> {
    return this.httpClient.get<ListResponseModel<RentalListModel>>(
      this.apiUrl + 'find-all'
    );
  }
  findAllByCustomerId(
    customerId: number
  ): Observable<ListResponseModel<MyRentalListModel>> {
    return this.httpClient.get<ListResponseModel<MyRentalListModel>>(
      this.apiUrl + 'find-all-by-customer-id/' + customerId
    );
  }

  findById(id: number): Observable<SingleResponseModel<RentalListModel>> {
    return this.httpClient.get<SingleResponseModel<RentalListModel>>(
  ...
  findActiveRentalByCarId(
    id: number
  ): Observable<SingleResponseModel<RentalListModel>> {
    return this.httpClient.get<SingleResponseModel<RentalListModel>>(
      this.apiUrl + 'find-active-rental-by-car-id/' + id
    );
  }
  addForIndividualCustomer(
    rental: CreateRentalModel
  ): Observable<SingleResponseModel<RentalAddResponseModel>> {
    return this.httpClient.post<SingleResponseModel<RentalAddResponseModel>>(
      this.apiUrl + 'add-for-individual-customer',
      rental
    );
  }
  addForCorporateCustomer(
    rental: CreateRentalModel
  ): Observable<SingleResponseModel<RentalAddResponseModel>> {
    return this.httpClient.post<SingleResponseModel<RentalAddResponseModel>>(
      this.apiUrl + 'add-for-corporate-customer',
      rental
    );
  }
