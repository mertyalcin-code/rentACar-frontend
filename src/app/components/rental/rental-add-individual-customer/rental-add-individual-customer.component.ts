...
export class RentalAddForIndividualCustomerComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.findCarById();
    this.authService.isCorporateCustomer()
      ? this.linkRouter.navigateByUrl(
          '/rental/corporate-customer/add/' + this.carId
        )
      : null;
  }
  //rental add form
  ...
  //adds rental and returns rentalId and Car Id
  addRental() {
  ...
  //sends request to add additional services
  ...
  //add item to basket for additional services
  ...
  //remove item to basket for additional services
  ...
  //checks basket for additional services
  ...
  //sends request for calculating total price
  calculateTotalPrice() {
